    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl p-4" style={{ background: "#2a090966", border: "2px solid #ff475766" }}>
        ...
        <p className="text-xs leading-relaxed" style={{ color: "#f0c8c8" }}>
          Real secondary market for Reg CF securities requires SEC Alternative Trading System (ATS) registration and FINRA broker-dealer membership.
          The 12-month Reg CF lock-up restriction applies to all securities shown. No real trades execute here.
          VisionScope is NOT a broker-dealer or ATS. This interface exists purely to illustrate how a secondary market for startup equity could function.
        </p>
      </div>

      {/* Info box */}
      <div className="rounded-xl p-4" style={{ background: "#06b6d411", border: "1px solid #06b6d422" }}>
        <div className="font-mono text-xs font-bold mb-1" style={{ color: "#06b6d4" }}>ℹ HOW THIS WOULD WORK</div>
        <p className="text-xs leading-relaxed" style={{ color: "#c4cde0" }}>
          This is how a "stock market for startups" could work. After the 12-month Reg CF lock-up, investors could buy and sell shares on a regulated secondary market. Health scores provide valuation benchmarks.
        </p>
      </div>
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "#0f1520", border: "1px solid #1b2540" }}>
        ...
        <div className="grid px-4 py-2 font-mono text-xs" style={{ gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr 1fr 1fr", background: "#131a28", borderBottom: "1px solid #1b2540", color: "#4e5f7d" }}>
          ...
          <div className="text-right">Lock-up</div>
        </div>
            ...
            <div key={c.id}>
              <button onClick={() => setSelected(isSelected ? null : c.id)}
                ...
                style={{
                  ...
                  background: isSelected ? "#1b254033" : i % 2 === 0 ? "#0c1220" : "#0f1520",
                  borderBottom: "1px solid #0a0e17",
                  borderLeft: isSelected ? `3px solid #2e6cf6` : "3px solid transparent",
                }}>
                <div>
                ...
                <div className="text-right">
                  <Pill label={c.status === "Live" ? "Locked" : "Open"} variant={c.status === "Live" ? "amber" : "green"} small />
                </div>
              </button>
              ...
              {isSelected && (
                <div className="px-4 pb-4 pt-2" style={{ background: "#0a0e17", borderBottom: "1px solid #1b2540" }}>
                  <div className="rounded-xl p-4 max-w-md" style={{ background: "#0f1520", border: "1px solid #1b2540" }}>
                    <div className="font-mono text-xs font-bold mb-3" style={{ color: "#c4cde0" }}>
                      Order Form — {c.id} · {c.name}
                    </div>
                    ...
                    <div className="flex gap-1 rounded-lg p-1 mb-3" style={{ background: "#131a28" }}>
                      <button onClick={() => setSide("buy")}
                        ...
                        style={{ background: side === "buy" ? "#052e1c" : "transparent", color: side === "buy" ? "#00d4aa" : "#4e5f7d" }}>
                        BUY
                      </button>
                      <button onClick={() => setSide("sell")}
                        className="flex-1 py-2 rounded font-mono text-xs font-bold transition-all"
                        style={{ background: side === "sell" ? "#2a0909" : "transparent", color: side === "sell" ? "#ff4757" : "#4e5f7d" }}>
                        SELL
                      </button>
                    </div>
                    ...
                    {shares > 0 && (
                      <div className="rounded-lg px-3 py-2 mb-3 flex justify-between"
                        style={{ background: "#131a28", border: "1px solid #1b2540" }}>
                    ...
                    <button className="w-full py-2.5 rounded-xl font-mono text-xs font-bold transition-all"
                      style={{
                        background: c.status === "Live" ? "#2a160066" : (side === "buy" ? "#052e1c" : "#2a0909"),
                        color: c.status === "Live" ? "#ffb830" : (side === "buy" ? "#00d4aa" : "#ff4757"),
                        border: `1px solid ${c.status === "Live" ? "#ffb83044" : side === "buy" ? "#00d4aa44" : "#ff475744"}`,
                      }}>
                      {c.status === "Live"
                        ? "Locked — 12mo hold required"
                        : `Preview ${side.toUpperCase()} order (concept only)`}
                    </button>
                  </div>
                </div>
              )}
