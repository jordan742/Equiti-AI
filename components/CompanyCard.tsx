...
export default function CompanyCard({ company: c, onClick }: { company: Company; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className="w-full text-left rounded-xl p-4 transition-all hover:border-blue-500"
      style={{ background: "#0f1520", border: "1px solid #1b2540" }}>
      <div className="flex gap-4">
        {/* Score ring */}
        <div className="shrink-0">
          <ScoreRing company={c} size={72} />
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Sector + platform */}
          <div className="font-mono text-xs mb-2" style={{ color: "#4e5f7d" }}>
          ...
          <div className="rounded-lg px-3 py-2 mb-3" style={{ background: "#06b6d40d", border: "1px solid #06b6d422" }}>
            <span className="font-mono text-xs" style={{ color: "#06b6d4" }}>
              {c.ai.conviction} conviction — {c.ai.verdict.toLowerCase()}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
            {[
              ...
              <div key={m.label} className="rounded px-2 py-1.5" style={{ background: "#131a28", border: "1px solid #1b2540" }}>
                <div className="font-mono text-xs" style={{ color: "#4e5f7d", fontSize: 9 }}>{m.label}</div>
