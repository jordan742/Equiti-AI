...
export function score(c: Company): number {
  ...
  if (runway > 18) s += 25;
  else if (runway > 12) s += 20;
  else if (runway > 6) s += 14;
  else if (runway > 3) s += 7;
  else s += 2;

  // 2. Revenue Growth (25pts)
  const g = c.growth;
  if (g > 80) s += 25;
  else if (g > 50) s += 20;
  else if (g > 25) s += 14;
  ...
  // 3. Debt Health (15pts): debt/assets
  const dr = c.xbrl.assets > 0 ? c.debt / c.xbrl.assets : 1;
  if (dr < 0.15) s += 15;
  else if (dr < 0.30) s += 11;
  else if (dr < 0.50) s += 6;
  else s += 2;

  // 4. Revenue Coverage (20pts): rev/burn
  const cov = c.burn > 0 ? c.revenue / c.burn : 0;
  if (cov > 1.0) s += 20;
  else if (cov > 0.5) s += 15;
  else if (cov > 0.2) s += 9;
  else if (cov > 0.05) s += 4;
  else s += 1;

  // 5. Current Ratio (15pts): assets/liabilities (proxy)
