...
const TICKER_ITEMS = ["ACTIVE DEALS: 220","WEEKLY VOL: $4.9M","NMSH $1.35 ▲8.0%","CBOT $3.15 ▲12.5%","HLNS $0.55 ▼8.3%","SYNK $5.10 ▲21.4%","TOTAL RAISED YTD: $8.4M","INVESTORS: 11,923"];
    ...
    <div className="min-h-screen flex flex-col" style={{background:"#0a0e17"}}>
      ...
      <div className="overflow-hidden py-1.5" style={{background:"#060d18",borderBottom:"1px solid #1b2540"}}>
        <div className="ticker-track flex gap-8 whitespace-nowrap" style={{width:"max-content"}}>
          {[...TICKER_ITEMS,...TICKER_ITEMS].map((item,i)=>(<span key={i} className="font-mono text-xs" style={{color:"#4e5f7d"}}>{item}</span>))}
        </div>
      </div>
      <nav className="px-4 flex gap-1 py-2" style={{background:"#0f1520",borderBottom:"1px solid #1b2540"}}>
        {(["Screener","Marketplace"] as Tab[]).map(t=>(
          <button key={t} onClick={()=>{setTab(t);setDetail(null);}} className="px-4 py-1.5 rounded-lg font-mono text-xs font-bold transition-all"
            style={{background:tab===t?"#1b2540":"transparent",color:tab===t?"#f0f4ff":"#4e5f7d",border:`1px solid ${tab===t?"#2e6cf644":"transparent"}`}}>{t}</button>
        ))}
      </nav>
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              <div className="flex gap-1 rounded-xl p-1" style={{background:"#0f1520",border:"1px solid #1b2540"}}>
                {(["Score","Runway","Raised","Growth"] as const).map(s=>(
                  <button key={s} onClick={()=>setSort(s)} className="px-3 py-1.5 rounded-lg font-mono text-xs transition-all"
                    style={{background:sort===s?"#1b2540":"transparent",color:sort===s?"#2e6cf6":"#4e5f7d"}}>{s}</button>
                ))}
              </div>
            </div>
